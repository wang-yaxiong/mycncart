...
class ControllerExtensionModulePPBraintreeButton extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/pp_braintree_button', 'user_tokens=' . $this->session->data['user_tokens'])
		);

		$data['action'] = $this->url->link('extension/module/pp_braintree_button', 'user_tokens=' . $this->session->data['user_tokens']);
		$data['cancel'] = $this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens'] . '&type=module');
		$data['layouts'] = $this->url->link('design/layout', 'user_tokens=' . $this->session->data['user_tokens']);
	...
	public function configure() {
		...
		if (!$this->user->hasPermission('modify', 'extension/extension/module')) {
			...
			$this->response->redirect($this->url->link('design/layout', 'user_tokens=' . $this->session->data['user_tokens']));
		}
	}
