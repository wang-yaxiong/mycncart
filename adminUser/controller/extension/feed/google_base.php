...
class ControllerExtensionFeedGoogleBase extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens'] . '&type=feed'));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens'] . '&type=feed')
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/feed/google_base', 'user_tokens=' . $this->session->data['user_tokens'])
		);

		$data['action'] = $this->url->link('extension/feed/google_base', 'user_tokens=' . $this->session->data['user_tokens']);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens'] . '&type=feed');
	...
	public function category() {
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('extension/feed/google_base/category', 'user_tokens=' . $this->session->data['user_tokens'] . '&page={page}');
