...
class ControllerExtensionPaymentPPExpress extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens'] . '&type=payment'));
		}

		$data['user_tokens'] = $this->session->data['user_tokens'];

		$data['button_configure'] = $this->url->link('extension/module/pp_button/configure', 'user_tokens=' . $this->session->data['user_tokens']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens'] . '&type=payment'),
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/payment/pp_express', 'user_tokens=' . $this->session->data['user_tokens']),
		);

		$data['action'] = $this->url->link('extension/payment/pp_express', 'user_tokens=' . $this->session->data['user_tokens']);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens'] . '&type=payment');

		$data['search'] = $this->url->link('extension/payment/pp_express/search', 'user_tokens=' . $this->session->data['user_tokens']);
		...
		$post_data = array(
			'return_url' => $this->url->link('extension/payment/pp_express', 'user_tokens=' . $this->session->data['user_tokens']),
		...
		if ($paypal_info) {
			...
			foreach ($results as $result) {
				$data['transactions'][] = array(
					...
					'view'           => $this->url->link('extension/payment/pp_express/info', 'user_tokens=' . $this->session->data['user_tokens'] . '&transaction_id=' . $result['transaction_id']),
					'refund'         => $this->url->link('extension/payment/pp_express/refund', 'user_tokens=' . $this->session->data['user_tokens'] . '&transaction_id=' . $result['transaction_id']),
					'resend'         => $this->url->link('extension/payment/pp_express/resend', 'user_tokens=' . $this->session->data['user_tokens'] . '&paypal_order_transaction_id=' . $result['paypal_order_transaction_id'])
				);
			}
		}
	...
	public function refund() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/payment/pp_express/refund', 'user_tokens=' . $this->session->data['user_tokens']),
		);

		//button actions
		$data['action'] = $this->url->link('extension/payment/pp_express/doRefund', 'user_tokens=' . $this->session->data['user_tokens']);
		$data['cancel'] = $this->url->link('extension/payment/pp_express', 'user_tokens=' . $this->session->data['user_tokens']);
			...
			if ($this->request->post['refund_full'] == 0 && $this->request->post['amount'] == 0) {
				...
				if ($paypal_order) {
					...
					if ($result == false) {
						...
						$this->response->redirect($this->url->link('sale/order/info', 'user_tokens=' . $this->session->data['user_tokens'] . '&order_id=' . $paypal_order['order_id']));
						...
						$this->response->redirect($this->url->link('extension/payment/pp_express/refund', 'user_tokens=' . $this->session->data['user_tokens'] . '&transaction_id=' . $this->request->post['transaction_id']));
					}
				} else {
	...
	public function search() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/payment/pp_express/search', 'user_tokens=' . $this->session->data['user_tokens']),
		);
		...
		$data['view_link'] = $this->url->link('extension/payment/pp_express/info', 'user_tokens=' . $this->session->data['user_tokens']);
	...
	public function info() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/payment/pp_express/info', 'user_tokens=' . $this->session->data['user_tokens'] . '&transaction_id=' . $this->request->get['transaction_id']),
		);
		...
		$data['view_link'] = $this->url->link('extension/payment/pp_express/info', 'user_tokens=' . $this->session->data['user_tokens']);
		$data['cancel'] = $this->url->link('extension/payment/pp_express/search', 'user_tokens=' . $this->session->data['user_tokens']);
		$data['user_tokens'] = $this->session->data['user_tokens'];
	...
	public function live() {
		...
		$this->response->redirect($this->url->link('extension/payment/pp_express', 'user_tokens=' . $this->session->data['user_tokens']));
	}
	...
	public function recurringButtons() {
		...
		if ($recurring['status'] == 2 || $recurring['status'] == 3) {
			$data['buttons'][] = array(
				...
				'link' => $this->url->link('extension/payment/pp_express/recurringCancel', 'order_recurring_id=' . $this->request->get['order_recurring_id'] . '&user_tokens=' . $this->request->get['user_tokens'])
			);
		}
	...
	public function connectRedirect() {
		if ($this->user->hasPermission('modify', 'extension/extension/payment') && $this->user->hasPermission('modify', 'extension/payment/pp_express')) {
			...
			if (isset($curl_response['url']) && !empty($curl_response['url'])) {
				...
				$this->response->redirect($this->url->link('marketplace/extension', 'user_tokens=' . $this->session->data['user_tokens']));
			}
		} else {
			$this->response->redirect($this->url->link('error/permission', 'user_tokens=' . $this->session->data['user_tokens']));
		}
	}
	...
	public function preferredSolution() {
		...
		if ($this->config->get('payment_pp_express_username') || $this->config->get('payment_pp_express_sandbox_username')) {
			$data['module_link'] = $this->url->link('extension/payment/pp_express', 'user_tokens=' . $this->session->data['user_tokens']);
		} else {
			if ($this->user->hasPermission('modify', 'extension/extension/payment')) {
				$data['connect_link'] = $this->url->link('extension/payment/pp_express/connectRedirect', 'user_tokens=' . $this->session->data['user_tokens']);
			}
		}
