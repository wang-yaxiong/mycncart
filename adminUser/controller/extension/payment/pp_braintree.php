...
class ControllerExtensionPaymentPPBraintree extends Controller {
	...
	public function index() {
		...
		$data['button_configure'] = $this->url->link('extension/module/pp_braintree_button/configure', 'user_tokens=' . $this->session->data['user_tokens']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/pp_braintree', 'user_tokens=' . $this->session->data['user_tokens'])
		);

		$data['action'] = $this->url->link('extension/payment/pp_braintree', 'user_tokens=' . $this->session->data['user_tokens']);
		...
		if (isset($this->request->get['retrieve_code'])) {
			...
			$post_data = array(
				'return_url' => $this->url->link('extension/payment/pp_braintree', 'user_tokens=' . $this->session->data['user_tokens']),
			...
			foreach ($transactions as $transaction) {
				...
				if ($transaction->customer['id']) {
					...
					if (isset($braintree_customer_id[2]) && is_numeric($braintree_customer_id[2])) {
						...
						if ($customer_info && $customer_info['email'] == $transaction->customer['email']) {
							$customer_url = $this->url->link('sale/customer/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&customer_id=' . (int)$braintree_customer_id[2]);
						}
					}
				}
				...
				if ($transaction->orderId) {
					...
					if ($order_info && $order_info['email'] == $transaction->customer['email']) {
						$order = $this->url->link('sale/order/info', 'user_tokens=' . $this->session->data['user_tokens'] . '&order_id=' . (int)$transaction->orderId);
					}
				}
	...
	public function preferredSolution() {
		...
		if ($this->config->get('payment_pp_braintree_status') || $this->config->get('payment_pp_braintree_merchant_id') || $this->config->get('payment_pp_braintree_access_token')) {
			$data['module_link'] = $this->url->link('extension/payment/pp_braintree', 'user_tokens=' . $this->session->data['user_tokens']);
		} else {
			if ($this->user->hasPermission('modify', 'extension/extension/payment')) {
				$data['connect_link'] = $this->url->link('extension/payment/pp_braintree/connectRedirect', 'user_tokens=' . $this->session->data['user_tokens']);
			}
		}
