...
class ControllerReportStatistics extends Controller {
	...
	public function ordersale() {
		...
		if ($this->validate()) {
			...
			$this->response->redirect($this->url->link('report/statistics', 'user_tokens=' . $this->session->data['user_tokens']));
		}
	...
	public function getList() {
		...
		foreach ($results as $result) {
			$data['statistics'][] = array(
				...
				'value' => $result['value'],
				'href'  => $this->url->link('report/statistics/' . str_replace('_', '', $result['code']), 'user_tokens=' . $this->session->data['user_tokens'])
			);
		}
