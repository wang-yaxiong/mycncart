...
class ControllerSettingStore extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('setting/store', 'user_tokens=' . $this->session->data['user_tokens'] . '&store_id=' . $this->request->get['store_id']));
		}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('setting/store/add', 'user_tokens=' . $this->session->data['user_tokens']);
		$data['delete'] = $this->url->link('setting/store/delete', 'user_tokens=' . $this->session->data['user_tokens']);
		...
		if ($page == 1) {
			$data['stores'][] = array(
				...
				'url'      => HTTP_CATALOG,
				'edit'     => $this->url->link('setting/setting', 'user_tokens=' . $this->session->data['user_tokens'])
			);
		}
		...
		foreach ($results as $result) {
			$data['stores'][] = array(
				...
				'name'     => $result['name'],
				'url'      => $result['url'],
				'edit'     => $this->url->link('setting/store/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&store_id=' . $result['store_id'])
			);
		}
		...
		$pagination->url = $this->url->link('setting/store', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['store_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('setting/store/add', 'user_tokens=' . $this->session->data['user_tokens'])
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_settings'),
				'href' => $this->url->link('setting/store/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&store_id=' . $this->request->get['store_id'])
			);
		}
		...
		if (!isset($this->request->get['store_id'])) {
			$data['action'] = $this->url->link('setting/store/add', 'user_tokens=' . $this->session->data['user_tokens']);
		} else {
			$data['action'] = $this->url->link('setting/store/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&store_id=' . $this->request->get['store_id']);
		}
