...
class ControllerLocalisationReturnAction extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/return_action', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/return_action', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('localisation/return_action/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('localisation/return_action/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['return_actions'][] = array(
				...
				'name'             => $result['name'],
				'edit'             => $this->url->link('localisation/return_action/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&return_action_id=' . $result['return_action_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/return_action', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=name' . $url);
		...
		$pagination->url = $this->url->link('localisation/return_action', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['return_action_id'])) {
			$data['action'] = $this->url->link('localisation/return_action/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('localisation/return_action/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&return_action_id=' . $this->request->get['return_action_id'] . $url);
		}

		$data['cancel'] = $this->url->link('localisation/return_action', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
