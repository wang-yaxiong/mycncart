...
class ControllerLocalisationGeoZone extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/geo_zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/geo_zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('localisation/geo_zone/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('localisation/geo_zone/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['geo_zones'][] = array(
				...
				'edit'        => $this->url->link('localisation/geo_zone/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&geo_zone_id=' . $result['geo_zone_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/geo_zone', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=name' . $url);
		$data['sort_description'] = $this->url->link('localisation/geo_zone', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=description' . $url);
		...
		$pagination->url = $this->url->link('localisation/geo_zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['geo_zone_id'])) {
			$data['action'] = $this->url->link('localisation/geo_zone/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('localisation/geo_zone/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&geo_zone_id=' . $this->request->get['geo_zone_id'] . $url);
		}

		$data['cancel'] = $this->url->link('localisation/geo_zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
