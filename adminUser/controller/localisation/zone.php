...
class ControllerLocalisationZone extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('localisation/zone/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('localisation/zone/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['zones'][] = array(
				...
				'code'    => $result['code'],
				'edit'    => $this->url->link('localisation/zone/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&zone_id=' . $result['zone_id'] . $url)
			);
		}
		...
		$data['sort_country'] = $this->url->link('localisation/zone', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=c.name' . $url);
		$data['sort_name'] = $this->url->link('localisation/zone', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=z.name' . $url);
		$data['sort_code'] = $this->url->link('localisation/zone', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=z.code' . $url);
		...
		$pagination->url = $this->url->link('localisation/zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['zone_id'])) {
			$data['action'] = $this->url->link('localisation/zone/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('localisation/zone/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&zone_id=' . $this->request->get['zone_id'] . $url);
		}

		$data['cancel'] = $this->url->link('localisation/zone', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
