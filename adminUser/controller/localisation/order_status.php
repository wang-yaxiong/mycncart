...
class ControllerLocalisationOrderStatus extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/order_status', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/order_status', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('localisation/order_status/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('localisation/order_status/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['order_statuses'][] = array(
				...
				'edit'            => $this->url->link('localisation/order_status/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&order_status_id=' . $result['order_status_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/order_status', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=name' . $url);
		...
		$pagination->url = $this->url->link('localisation/order_status', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['order_status_id'])) {
			$data['action'] = $this->url->link('localisation/order_status/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('localisation/order_status/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&order_status_id=' . $this->request->get['order_status_id'] . $url);
		}

		$data['cancel'] = $this->url->link('localisation/order_status', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
