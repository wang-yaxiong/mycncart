...
class ControllerLocalisationReturnReason extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/return_reason', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/return_reason', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('localisation/return_reason/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('localisation/return_reason/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['return_reasons'][] = array(
				...
				'name'             => $result['name'],
				'edit'             => $this->url->link('localisation/return_reason/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&return_reason_id=' . $result['return_reason_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/return_reason', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=name' . $url);
		...
		$pagination->url = $this->url->link('localisation/return_reason', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['return_reason_id'])) {
			$data['action'] = $this->url->link('localisation/return_reason/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('localisation/return_reason/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&return_reason_id=' . $this->request->get['return_reason_id'] . $url);
		}

		$data['cancel'] = $this->url->link('localisation/return_reason', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
