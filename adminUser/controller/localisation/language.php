...
class ControllerLocalisationLanguage extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/language', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/language', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('localisation/language/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('localisation/language/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['languages'][] = array(
				...
				'edit'        => $this->url->link('localisation/language/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&language_id=' . $result['language_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/language', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=name' . $url);
		$data['sort_code'] = $this->url->link('localisation/language', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=code' . $url);
		$data['sort_sort_order'] = $this->url->link('localisation/language', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=sort_order' . $url);
		...
		$pagination->url = $this->url->link('localisation/language', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['language_id'])) {
			$data['action'] = $this->url->link('localisation/language/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('localisation/language/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&language_id=' . $this->request->get['language_id'] . $url);
		}

		$data['cancel'] = $this->url->link('localisation/language', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
