...
class ControllerCatalogOption extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/option', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/option', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('catalog/option/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('catalog/option/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['options'][] = array(
				...
				'edit'       => $this->url->link('catalog/option/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&option_id=' . $result['option_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/option', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=od.name' . $url);
		$data['sort_sort_order'] = $this->url->link('catalog/option', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=o.sort_order' . $url);
		...
		$pagination->url = $this->url->link('catalog/option', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['option_id'])) {
			$data['action'] = $this->url->link('catalog/option/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('catalog/option/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&option_id=' . $this->request->get['option_id'] . $url);
		}

		$data['cancel'] = $this->url->link('catalog/option', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
