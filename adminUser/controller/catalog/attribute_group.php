...
class ControllerCatalogAttributeGroup extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/attribute_group', 'user_tokens=' . $this->session->data['user_tokens'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/attribute_group', 'user_tokens=' . $this->session->data['user_tokens'] . $url)
		);

		$data['add'] = $this->url->link('catalog/attribute_group/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		$data['delete'] = $this->url->link('catalog/attribute_group/delete', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		...
		foreach ($results as $result) {
			$data['attribute_groups'][] = array(
				...
				'edit'               => $this->url->link('catalog/attribute_group/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&attribute_group_id=' . $result['attribute_group_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/attribute_group', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=agd.name' . $url);
		$data['sort_sort_order'] = $this->url->link('catalog/attribute_group', 'user_tokens=' . $this->session->data['user_tokens'] . '&sort=ag.sort_order' . $url);
		...
		$pagination->url = $this->url->link('catalog/attribute_group', 'user_tokens=' . $this->session->data['user_tokens'] . $url . '&page={page}');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['attribute_group_id'])) {
			$data['action'] = $this->url->link('catalog/attribute_group/add', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
		} else {
			$data['action'] = $this->url->link('catalog/attribute_group/edit', 'user_tokens=' . $this->session->data['user_tokens'] . '&attribute_group_id=' . $this->request->get['attribute_group_id'] . $url);
		}

		$data['cancel'] = $this->url->link('catalog/attribute_group', 'user_tokens=' . $this->session->data['user_tokens'] . $url);
