...
//            ($query->rows)[$k]['date_available'] = strtotime($v['date_available']);
            ...
            ($query->rows)[$k]['href'] = $this->url->link('account/ebaypro', 'product_id=' . $v['product_id']);
        ...
        unset($query->rows);
//        echo '<pre><hr>';
//        print_r($this->getProductImages());die;
//        echo '</pre><hr>';
...
//
	public function forProduct()
    {
        ...
        foreach($ress->rows as $k=>$v){
            ...
            $endtime = strtotime($v['date_available']);
           if($oldtime<=$now){
               if($v['stock_status_id']>7){
                   //更新上架状态update
